...
describe('script preprocessor', () => {
    test('skips content without @derive', async () => {
        ...
        assert.strictEqual(result, undefined, 'Should return undefined for content without @derive');
    });
    ...
    test('skips non-TypeScript by default', async () => {
        ...
        assert.strictEqual(result, undefined, 'Should skip JavaScript blocks by default');
    });
...
class Point {
    ...
    y: number;
        ...
        if (result !== undefined) {
            assert.ok(result.code.includes('debug') || result.code.includes('clone'),
